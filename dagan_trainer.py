...
class DaganTrainer:
    def __init__(
        ...
        should_display_generations=False,
    ):
        #self.device = device
    ...
    def train(self, data_loader, epochs, val_images=None, save_training_gif=True):
        if self.tracking_images is None and self.num_tracking_images > 0:
            ...
            self.tracking_images = torch.stack(self.tracking_images).cuda()
            self.tracking_z = torch.randn((self.num_tracking_images, self.g.z_dim)).cuda()
    ...
    def sample_generator(self, input_images, z=None):
        if z is None:
            z = torch.randn((input_images.shape[0], self.g.z_dim)).cuda()
    ...
    def display_generations(self, data_loader, val_images):
        ...
        z = torch.randn((len(images), self.g.z_dim)).cuda()
        inp = torch.stack(images).cuda()
